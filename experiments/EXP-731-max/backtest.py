...
Iteration on EXP-730-max. Changes:
  1. IV rank filter REMOVED (always trade)
  2. Fixed Mon/Wed/Fri entry schedule (3x/week)
  3. Event blocking: FOMC only (CPI/NFP allowed)
Target: 150+ trades/year vs EXP-730's 16/year.
"""
...
# Trade frequency — fixed 3x/week on Mon/Wed/Fri
TRADES_PER_WEEK_MAX = 3
ENTRY_DAYS = {0, 2, 4}  # Monday=0, Wednesday=2, Friday=4
...
# Filter thresholds — NO IV RANK FILTER in EXP-731
IV_RANK_THRESHOLD = 0.0   # DISABLED: always trade regardless of IV rank
...
# Block FOMC days ONLY (CPI/NFP allowed in EXP-731)
BLOCKED_DATES = set(FOMC_DATES)
...
def run_backtest(seed: int = 42) -> Dict[str, Any]:
    ...
    for i, today in enumerate(trading_days):
        ...
        # ── Entry filters ──────────────────────────────────────────────
        # 1. Fixed Mon/Wed/Fri schedule
        if today.weekday() not in ENTRY_DAYS:
            continue
        ...
        # 3. NO IV rank filter (EXP-731 change: always trade)
        # iv_rank check removed

        # 4. Event filter — FOMC only
        if today in BLOCKED_DATES:
            continue

        # 5. Regime filter: skip crash, reduce in high_vol
        if regime == "crash":
            continue
    ...
    summary = {
        "experiment": "EXP-731-max",
        "name": "Short DTE Theta Decay — High Frequency",
...
<head>
<meta charset="utf-8"/>
<title>EXP-731-max: Short DTE High Frequency</title>
<style>
...
<body>
<h1>EXP-731-max: Short DTE High Frequency</h1>
...
if __name__ == "__main__":
    print("Running EXP-731-max backtest...")
    ...
    # Print summary
    print(f"\n{'='*60}")
    print(f"EXP-731-max: Short DTE High Frequency — Results")
    print(f"{'='*60}")
